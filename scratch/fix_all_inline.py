...
for t in doc.tables:
    for row in t.rows:
        ...
        # Fix the other buggy spaced tags
...
# Fix isolated spaces in period.rows run
for t in doc.tables:
    for row in t.rows:
        if 'period.rows' in ''.join(c.text for c in row.cells):
            for c in row.cells:
                for p in c.paragraphs:
...
print("Complete fix applied with inline if!")
