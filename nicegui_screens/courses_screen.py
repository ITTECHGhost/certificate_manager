...
class CoursesScreen:
    ...
    def _get_filtered_data(self) -> list[dict]:
        ...
        if self.filter_dept > 0:
            rows = [r for r in rows if r.get("department_id") == self.filter_dept]
        ...
        with ui.row().classes("w-full items-center justify-between p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-default)] gap-4 flex-nowrap overflow-hidden hover:border-[var(--color-accent)] transition-all shadow-sm"):
            ...
            with ui.row().classes("items-center gap-3 shrink-0 flex-nowrap"):
                with ui.column().classes("items-center gap-0 shrink-0"):
                    ...
                    ui.label(dept).classes("text-xs font-bold px-2.5 py-1 rounded-lg bg-[var(--bg-card)] border border-[var(--border-default)] app-text-primary truncate max-w-[150px]")
    ...
    def show_edit_view(self, row: dict | None = None, mode: str = "add"):
        ...
        shared_state = {"is_shared": existing_data.get("is_shared", False)}
        selected_depts = []
        if shared_state["is_shared"] and cid:
            selected_depts = self.repo.get_shared_dept_ids(cid)
                ...
                with ui.column().classes("w-full gap-6 overflow-y-auto pr-1"):
                            ...
                            if is_shared_chk.value:
                                ...
                                with ui.row().classes("gap-4 flex-wrap w-full p-4 rounded-xl bg-[var(--bg-main)] border border-[var(--border-default)]"):
                                    for did, dname in dept_opts.items():
                                        ui.checkbox(dname, value=(did in selected_depts)).on('update:model-value', lambda e, d=did: selected_depts.append(d) if e.value else selected_depts.remove(d) if d in selected_depts else None)
                            else:
                                dept_sel = UI.select(
                                    "القسم / Department",
                                    options=dept_opts,
                                    value=existing_data.get("department_id") if existing_data and existing_data.get("department_id") else (list(dept_opts.keys())[0] if dept_opts else None)
                    ...
                    with ui.row().classes("w-full justify-end gap-3 pt-4 border-t border-[var(--border-default)] shrink-0"):
                        def save_action():
                            ...
                            if not n_ar:
                                ui.notify("اسم المادة بالعربية مطلوب / Arabic name is required", type="warning")
                                return
                            if not n_en:
                                ui.notify("اسم المادة بالإنكليزية مطلوب / English name is required", type="warning")
                                return
                            ...
                            if is_shared:
                                ...
                                payload["department_id"] = self.single_dept_sel.value
                                payload["shared_dept_ids"] = []

                            try:
                                if mode == "add":
                                    self.repo.insert(data=payload)
                                    ui.notify("تمت الإضافة بنجاح / Course added", type="positive")
                                else:
                                    self.repo.update(course_id=cid, data=payload)
                                    ui.notify("تم التعديل بنجاح / Course updated", type="positive")
                                self.show_list_view()
    ...
    def confirm_delete(self, row: dict):
        ...
        with dialog, UI.card().classes("p-6 gap-6 w-full max-w-md bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)]"):
            ...
            ui.label(
                f"هل أنت تأكد من رغبتك في حذف المادة: ({row.get('name_ar', '')})؟\n"
